...
class OutFileClass {
  private :
    ...
    std::map<std::string, TProfile*> tp1_;
  public :
    ...
    void AddTProfile(TProfile* tp1) { tp1_.insert( pair<std::string, TProfile*>(std::string(tp1->GetName()), tp1)) ; }
    void AddTProfile(std::string str, TProfile* tp1) { tp1_.insert( pair<std::string, TProfile*>(str, tp1)) ; }
    ...
    TH1* GetTH1(std::string str) {
      ...
      else { std::cout<<"Can not find TH1F"<<std::endl ; return nullptr; }
    }
    TProfile* GetTProfile(std::string str) {
      auto value = tp1_.find(str);
      if ( value != tp1_.end() ) return value->second;
      else { std::cout<<"can not find Profile"<<std::endl; return nullptr; }
    }  
    ...
    void AllHistWrite() { 
      ...
      for( auto i = tp1_.begin() ; i != tp1_.end(); ++i) {
        i->second->Write();
      }
    } 
    void AllTreeWrite() {
...
void AnalyseEvents(ExRootTreeReader *treeReader, OutFileClass& ofc)
{
    ...
    for( unsigned int jet_idx = 0 ; jet_idx < selJet.size() ; ++jet_idx) {
      ...
      for( unsigned int firstTrack = 0 ; firstTrack< tracks.size()-1 ; firstTrack++) {
        for( unsigned int secondTrack = firstTrack+1 ; secondTrack< tracks.size() ; secondTrack++) {
          ...
          if ( abs(track1PID)==321 ) {
            GenParticle* kaon = (GenParticle*) tracks[firstTrack]->Particle.GetObject();
            if ( kaon != nullptr) ofc.GetTProfile("kaon_pt_drop")->Fill( tracks[firstTrack]->PT, tracks[firstTrack]->PT/ kaon->PT ); 
          } 
          if ( abs(track1PID)==211 ) {
            GenParticle* pion = (GenParticle*) tracks[firstTrack]->Particle.GetObject();
            if ( pion != nullptr) ofc.GetTProfile("pion_pt_drop")->Fill( tracks[firstTrack]->PT, tracks[firstTrack]->PT/ pion->PT ); 
          } 
  ...
  std::cout<<TString::Format("nD0 : %d",nD0)<<std::endl;
}
...
void BookingHist(OutFileClass& ofc)
{
  ...
  TProfile* h3 = new TProfile("kaon_pt_drop","Kaon pt drop", 2000,0,20);
  ofc.AddTProfile(h3);
  TProfile* h4 = new TProfile("pion_pt_drop","Pion pt drop", 2000,0,20);
  ofc.AddTProfile(h4);
  TH1F* h5 = new TH1F("d0_mass","Invariant mass of D0 Candidate", 100,1.6,2.2);
  ofc.AddTH1(h5);
