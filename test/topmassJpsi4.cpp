...
class SecVtx  { 
  public :
    ...
    SecVtx(Track* trackA, Track* trackB, TClonesArray* branchParticle=nullptr, Track* trackC=nullptr, float dau1_V3D=1000.0f, float dau2_V3D=1000.0f){
      init();
      ...
      if ( trackC != nullptr) softlepTLV_ = TracktoTLV(trackC);
      else softlepTLV_= TLorentzVector();
    ...
    SecVtx(SecVtx* secvtxA, Track* trackB, TClonesArray* branchParticle=nullptr, Track* trackC=nullptr, float dau1_V3D=1000.0f, float dau2_V3D=1000.0f, float dau3_V3D=1000.0f) {
      init();
      float track2_mass = 0;
      auto trackB_TLV = TracktoTLV(trackB,211);
    ...
    TLorentzVector softLeptonTLV() { return softlepTLV_;}
...
class saveData {
  public :
    ...
    void fillLSV(QLV* lep1_QLV, QLV* lep2_QLV, SecVtx* svx){
      ...
      TLorentzVector lsvx1 = pair.first + svx->softLeptonTLV();
  ...
  for(int entry = 0; entry < allEntries; ++entry)
  {
    ...
    for( unsigned int jet_idx = 0 ; jet_idx < selJet.size() ; ++jet_idx) {
        ...
        for( unsigned int secondTrack = firstTrack+1 ; secondTrack< tracks.size() ; secondTrack++) {
          ...
          if (  pidMul == -169 ) {
            ...
            if ( jpsiCand.M()>2 && jpsiCand.M()<4  &&  abs(3.096-jpsiCand.M())<abs(3.096-best_jpsi_mass) ) {
              ...
              jpsi[jet_idx] = new SecVtx( tracks[firstTrack], tracks[secondTrack], branchParticle, nullptr, track1_v3D);
      ...
      for( unsigned int softleptonTrack = 0 ; softleptonTrack < tracks.size() ; softleptonTrack++) {
        // If D0 + l^-    -> kaon- pion+ l-
        if ( (tracks[softleptonTrack]->PID == 11 || tracks[softleptonTrack]->PID ==13) )   { nsoftlep++;  isD0    = true ; softlepTrack = tracks[softleptonTrack]; break; } // D0,
        // If D0bar + l^+ -> kaon+ pion- l^+,
        if ( (tracks[softleptonTrack]->PID == -11 || tracks[softleptonTrack]->PID ==-13) ) { nsoftalep++; isD0    = false; softlepTrack = tracks[softleptonTrack]; break; } // D0bar,
      }
      // Soft lepton check.
      ...
      for( unsigned int firstTrack = 0 ; firstTrack< tracks.size()-1 ; firstTrack++) {
        for( unsigned int secondTrack = firstTrack+1 ; secondTrack< tracks.size() ; secondTrack++) {
          ...
          if ( abs(1.864 - d0Cand.M()) < abs(1.864-best_d0_mass)) {
            ...
            d0[jet_idx] = new SecVtx( tracks[firstTrack], tracks[secondTrack], branchParticle, softlepTrack, track1_v3D );
          ...
          for( unsigned int pionTrack = 0 ; pionTrack < tracks.size() ; pionTrack++) {
            ...
            if ( tracks[pionTrack]->Charge == tracks[secondTrack]->Charge ) {
              auto dstarCand = d0Cand+ TracktoTLV(tracks[pionTrack],211);
              ...
              if ( diffMass > 0.135 && diffMass<0.170 && abs(0.145-diffMass)<abs(0.145-best_dstar_diffmass)) {
                ...
                dstar[jet_idx] = new SecVtx( d0[jet_idx], tracks[pionTrack], branchParticle, softlepTrack, track1_v3D, track2_v3D, track3_v3D );
