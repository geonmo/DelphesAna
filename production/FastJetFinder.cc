...
void FastJetFinder::Init()
{
  ...
  fJetChargeOption = GetInt("JetChargeOption",0);
...
void FastJetFinder::Process()
{
  ...
  Int_t charge; 
  Double_t charge_den, charge_num; 
  Double_t rho = 0.0;
  PseudoJet jet, area;
  ...
  for(itOutputList = outputList.begin(); itOutputList != outputList.end(); ++itOutputList)
  {
      ...
      if ( constituent->Charge!=0) {
        ...
        switch(fJetChargeOption){
          case 0:
            ...
            break;
          case 1:
            weight = constituent->Momentum.Pt();
            break;
        }
        charge_num += constituent->Charge*weight;
        charge_den += weight;
      }
    ...
    if (charge_den >0.0 ) {
      charge = (int)(  abs(charge_num)/abs(charge_den)+0.5 );
